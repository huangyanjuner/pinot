...
public abstract class BaseStarTreeIndexTest {
  ...
  // TODO: fix the backward-compatible issue for old star-tree segment in 'data/starTreeSegment.tar.gz'
  protected void testHardCodedQueries() throws Exception {
    ...
    for (String query : getHardCodedQueries()) {
      ...
      Map<List<Integer>, List<Double>> expectedResult = computeUsingRawDocs();
      Map<List<Integer>, List<Double>> actualResult = computeUsingAggregatedDocs();
      Assert.assertEquals(expectedResult, actualResult, "Comparison failed for query: " + query);
    }
  }
  ...
  private Map<List<Integer>, List<Double>> computeUsingAggregatedDocs() throws Exception {
    ...
    filterOperator = new FilterPlanNode(_segment, _brokerRequest).run();
    return compute(filterOperator);
  }
